...
def error_is(
    ...
    """Record a failure when target is not exc or one of its causes."""
    seat.helper()
...
def error_is_not(
    ...
    """Record a failure when target is exc or one of its causes."""
    seat.helper()
...
def error_as(
    ...
    Record a failure when the chain holds none.
    """
    seat.helper()
...
def raises(seat: Seat, fn: Callable[[], Any], msg: str) -> BaseException | None:
    ...
    Record a failure when fn returns without raising.
    """
    seat.helper()
...
def does_not_raise(seat: Seat, fn: Callable[[], Any], msg: str) -> None:
    """Run fn and record when it raises."""
    seat.helper()
...
def honours_cancellation(
    ...
    """Record a failure when a cancelled subject does not raise CancelledError."""
    seat.helper()
    behaviour.honours_cancellation(seat, _MODE, fn, msg)


def honours_deadline(seat: Seat, fn: Callable[[], Awaitable[Any]], msg: str) -> None:
    """Record a failure when a subject given no time does not time out."""
    seat.helper()
...
def is_pure(
    ...
    *options: Option,
    ...
    """Record a failure when observed state changes across a call."""
    seat.helper()
    behaviour.is_pure(seat, _MODE, observe, fn, msg, *options)


def none_handle_safe(seat: Seat, fn: Callable[[Any], Any], msg: str) -> None:
    """Record a failure when a subject given None where a handle goes crashes."""
    seat.helper()
...
def eventually(
    ...
    Record a failure with the last attempt's failure when it never passes.
    """
    seat.helper()
...
def eventually_true(
    ...
    Record a failure when it never becomes true.
    """
    seat.helper()
...
def no_task_leaks(seat: Seat, msg: str) -> Callable[[], None]:
    ...
    The check records when a task started after this call is still
    running.
    """
    seat.helper()
